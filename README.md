...
experience, education, certificates, contact form) backed by a real database, plus
an admin dashboard where you log in and edit everything without touching code.
...
- **backend/** is a REST API. It stores your profile, skills, projects,
  experiences, education, certificates, and contact-form messages in PostgreSQL,
  and handles admin login with a JWT stored in an httpOnly cookie.
...
# optionally set NEXT_PUBLIC_SITE_URL for SEO metadata
...
Everything on the homepage — your name, bio, skills, projects, experience,
education, certificates, social links, and availability status — comes from
the database, editable at `/admin`. There is no need to touch component code
to update your info; just log in and use the dashboard.

Admin sections:
- **Profile** — name, title, tagline, bio, education info, social links
- **Skills** — categorized technology tags
- **Projects** — featured projects with images, tags, and links
- **Experience** — work history timeline entries
- **Education** — education timeline entries
- **Certificates** — certifications and learning credentials
...
Fly.io, a VPS, etc.). Set these environment variables:
...
- `NEXT_PUBLIC_SITE_URL` — your public site URL for SEO metadata
...
- Admin routes on the backend all require a valid session cookie
  ...
  experience, education, certificates, and profile cannot be written to
  without logging in.
